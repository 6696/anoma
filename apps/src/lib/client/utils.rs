...
pub async fn join_network(
    ...
    let base_dir_name = base_dir.file_name();
    let wasm_dir_name = wasm_dir.as_ref().and_then(|dir| {
        dir.file_name()
    ...
    let (unpack_dir, non_default_dirs) = if base_dir_name
        .map(|os_str| os_str.to_string_lossy().to_string())
        != Some(config::DEFAULT_BASE_DIR.to_string())
    {
        (base_dir.join(temp_dir), true)
    } else if wasm_dir_name != Some(config::DEFAULT_WASM_DIR.to_string()) {
    ...
    if dbg!(non_default_dirs) {
        ...
        archive_build
            .append_dir_all(
                ...
                dbg!(&base_dir.strip_prefix(&cwd).unwrap()),
        ...
        if let Some(wasm_dir) = wasm_dir.as_ref() {
            archive_build
                .append_dir_all(
                    ...
                    dbg!(wasm_dir.strip_prefix(&cwd).unwrap()),
